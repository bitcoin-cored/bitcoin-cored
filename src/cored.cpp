...
bool AppInit(int argc, char *argv[]) {
    ...
    // Parameters
    //
    // If Qt is used, parameters/core.conf are parsed in qt/bitcoin.cpp's
    // main()
    ...
    try {
        ...
        if (fCommandLine) {
            fprintf(stderr, "Error: There is no RPC client functionality in "
                            "cored anymore. Use the core-cli utility "
                            "instead.\n");
            exit(EXIT_FAILURE);
        }
        // -server defaults to true for cored but not for the GUI so do this
        // here

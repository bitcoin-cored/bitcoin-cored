...
BOOST_AUTO_TEST_CASE(cash_difficulty_test) {
    ...
    CBlockHeader blkHeaderDummy;
    uint32_t nBits =
        GetNextCoreWorkRequired(&blocks[2049], &blkHeaderDummy, params);

    // Difficulty stays the same as long as we produce a block every 10 mins.
    for (size_t j = 0; j < 10; i++, j++) {
        blocks[i] = GetBlockIndex(&blocks[i - 1], 600, nBits);
        BOOST_CHECK_EQUAL(
            GetNextCoreWorkRequired(&blocks[i], &blkHeaderDummy, params),
            nBits);
    }
    ...
    BOOST_CHECK_EQUAL(
        GetNextCoreWorkRequired(&blocks[i++], &blkHeaderDummy, params), nBits);
    ...
    // impact.
    blocks[i] = GetBlockIndex(&blocks[i - 1], 6000, nBits);
    nBits = GetNextCoreWorkRequired(&blocks[i++], &blkHeaderDummy, params);
